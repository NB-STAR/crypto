...
首先，我们来简单介绍一下 **Coppersmith method** 方法，该方法由 Don Coppersmith ( 外网链接：https://en.wikipedia.org/wiki/Don_Coppersmith ) 提出，可以用来找到单变量或者二元变量的多项式在模某个整数下的根，这里我们主要以单变量为主，假设我们有如下的一个在模 N 意义下的多项式 F
...
**Coppersmith method** 主要是通过 Lenstra–Lenstra–Lovász lattice basis reduction algorithm ( 外网链接：https://en.wikipedia.org/wiki/Lenstra%E2%80%93Lenstra%E2%80%93Lov%C3%A1sz_lattice_basis_reduction_algorithm )（LLL）方法来找到与该函数具有相同根 $x_0$ 但有更小系数的多项式。关于更加详细的介绍，请自行搜索。
...
有兴趣的可以进一步阅读 https://www.iacr.org/archive/pkc2005/33860001/33860001.pdf) 以及 [paper (  A New Related Message Attack on RSA]  ( 外网链接：https://www.cs.unc.edu/~reiter/papers/1996/Eurocrypt.pdf ) 这里暂不做过多的讲解。
...
关于 `small_roots` 的使用，可以参考 SAGE 说明 ( 外网链接：http://doc.sagemath.org/html/en/reference/polynomial_rings/sage/rings/polynomial/polynomial_modn_dense_ntl.html#sage.rings.polynomial.polynomial_modn_dense_ntl.small_roots )。
