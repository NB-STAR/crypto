...
- MD5算法的输入可以是任意长度的消息x，对输入消息按512位的分组为单位进行处理，输出128位的散列值MD(x)。
...
1. 打开参考代码存放目录\Home\crypto\Documents\course，找到MD5文件夹，包含文件`md5.h`、`md5.cpp`、`main.cpp`
![](files/2018-04-24-12-03-46.png)
2.在文件存放目录下右键打开terminal, 执行如下命令
`g++ main.cpp md5.cpp -o md5_sample && ./md5_sample`
3. 得到经过MD5加密后的值
`md5 of 'grape': b781cbb29054db12f88f08c6e161c199`
![](files/2018-04-24-14-29-22.png)
此处的`main.cpp`只是测试用的代码，具体的功能同学们可以拓展。

### 【总结】

- 通过本节实验的学习，实现了用MD5算法进行加密。